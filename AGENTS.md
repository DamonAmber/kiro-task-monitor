...
- **极简模式（compactMode）**：config `compactMode`(默认关) + `compactBounds`。普通/极简两种尺寸与位置**各自记忆**
  ...
  托盘右键菜单也能切（切后 `poll()` 立即把 config 推给渲染层）。渲染层给 `#app` 加/去 `.compact` 类，样式全在
  `styles.css` 的 `#app.compact ...`（单行、隐藏 `.card-meta`/`.title`/`.src`、来源改用 `data-source` 左边框、显示 `.card-mini-time`、隐藏用量条）。
...
- **手动**（每次功能变更必须更新）：功能描述、状态图例、「怎么使用」、**更新日志（`#changelog`）**、系统要求、首屏截图。这些是静态 HTML。
  - 更新日志规则：**新功能单独列条、写清楚**（加「新增」绿标）；**Bug 修复不写细节**，用「修复了若干已知问题」之类模糊措辞（可合并进相邻功能版本的条目）。
