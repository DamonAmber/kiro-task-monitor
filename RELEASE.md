...
- [ ] **落地页已同步**：若本次有新增/改动/移除的用户可见功能，已相应更新 `docs/index.html`
      的功能卡片、状态图例、「怎么使用」等文案（对外只讲用法、不讲实现原理）。
- [ ] **更新日志已加条目**（`#changelog`）：新功能单独列、写清楚（加「新增」标）；纯 Bug 修复用
      「修复了若干已知问题」之类模糊措辞、不写细节（可并入相邻功能版本条目）。
- [ ] **截图已更新且脱敏**：界面有变化则已重做 `docs/screenshot.png`，且**只用虚构示例数据、无真实项目名**。
- [ ] README 的功能/说明与新版本一致。
- [ ] **性能基准通过**：`npm run perf` 退出码为 0（稳态/增量单轮 <50ms、合成 4MB 冷启动 <3s）。
      这是小而美工具的底线——`poll()` 每 2s 在主线程同步跑，热路径一旦有同步重活整个 UI 会卡死
      （v0.11.0 曾因此翻车）。**改了扫描/解析/轮询/进程或 DB 读取等热路径时尤其必须跑并通过**，再打 tag。
- [ ] 本地自测通过（`npm start` 起浮窗肉眼确认；改了判定逻辑则 `npm run watch:once` 核对）。

> 下载链接与版本号由页面 JS 自动读取最新 Release，无需手改；**功能描述与截图不会自动更新，必须手动同步**。
