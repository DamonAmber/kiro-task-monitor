...
**卡住检测是上下文感知的,不是死板的固定时长**:纯靠"多久没写入"很容易把"正在跑长命令/构建/测试"误判成卡住。为此 `deriveState` 会用 `tool_call` / `tool_result` 的 `toolCallId` 配对,判断当前是否有**在途工具**(发起了 `tool_call` 但还没收到 `tool_result`):
- 有工具在执行 → 属正常运行,用更长的宽限(`toolStuckSeconds`,默认 900s)才判卡住;
- 没有工具在执行、纯粹长时间无任何写入 → 才用较短阈值(`stuckSeconds`,默认 240s)判卡住。

文件层面无法 100% 区分"真卡死"与"跑得慢"(两者都表现为不再写入),但结合"是否有在途工具 + 更合理的阈值",能大幅减少误报。
...
| 卡住阈值·工具执行中（秒） | 有工具在跑（长命令/构建/测试）时用的更长宽限 | 900 |
